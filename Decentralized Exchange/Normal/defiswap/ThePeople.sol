...
interface IMigratorChef {
    ...
    // SushiSwap must mint EXACTLY the same amount of KingSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// ThePeople is the master of King. ThePeople can make the King.
//
...
contract ThePeople is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accSushiPerShare; // Accumulated KINGs per share, times 1e12. See below.
    }
    // The KING TOKEN!
    ...
    function updatePool(uint256 _pid) public {
        ...
        king.mint(devaddr, sushiReward.div(10));
        king.mint(address(this), sushiReward);
    ...
    function safeSushiTransfer(address _to, uint256 _amount) internal {
        uint256 sushiBal = king.balanceOf(address(this));
        if (_amount > sushiBal) {
            king.transfer(_to, sushiBal);
        } else {
